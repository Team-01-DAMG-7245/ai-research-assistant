...
class TaskManager:
        ...
        with _db_lock:
            ...
            if not table_exists:
                ...
                if "needs_hitl" not in columns:
                    cursor.execute(
                        "ALTER TABLE tasks ADD COLUMN needs_hitl INTEGER DEFAULT 0"
                    )
    ...
    def mark_task_failed(self, task_id: str, error_message: str):
        ...
        with _db_lock:
            ...
            cursor.execute(
                ...
                WHERE task_id = ?
            """,
                (TaskStatus.FAILED.value, error_message, "Task failed", now, task_id),
            )
    ...
    def approve_review(self, task_id: str) -> bool:
        ...
        with _db_lock:
            ...
            cursor.execute(
                ...
                WHERE task_id = ?
            """,
                (TaskStatus.COMPLETED.value, "Report approved", now, task_id),
            )
    ...
    def reject_review(
        self, task_id: str, rejection_reason: str
        ...
        logger.info(
            f"Rejected report for task {task_id}: {rejection_reason}. Preparing for regeneration."
        )
