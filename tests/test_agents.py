...
def test_search_agent(
    ...
    mock_semantic_search.return_value = mock_pinecone_results[:2]  # Return 2 results per query
    
    # Run search agent
    result_state = search_agent_node(sample_state)
    
    # Verify results
    assert result_state["current_agent"] == "search_agent"  # search_agent sets this value
...
def test_search_agent_empty_query(
    ...
    assert "search_queries" not in result_state or len(result_state.get("search_queries", [])) == 0
...
def test_verify_citations():
    ...
    report = "This is a claim [Source 1]. Another claim [Source 2]. Invalid [Source 10]."
    num_sources = 5
...
def test_validation_agent_high_confidence(
    ...
    sample_state["report_draft"] = "This is a well-cited report [Source 1] [Source 2] [Source 3]."
...
def test_workflow_error_handling(
    ...
    assert final_state.get("error") is not None or len(final_state.get("search_results", [])) == 0
