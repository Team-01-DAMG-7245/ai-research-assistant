...
def completed_task_id(client: TestClient, test_db) -> str:
    ...
    task_id = task_manager.create_task(
        query="Test query for completed task",
        user_id="test_user"
    )
...
def pending_review_task_id(client: TestClient, test_db) -> str:
    ...
    task_manager = get_task_manager()
    task_id = task_manager.create_task(
        query="Test query for pending review",
        user_id="test_user"
    )
...
def test_submit_query_with_different_depths(client: TestClient):
    ...
    for depth in depths:
        ...
        response = client.post(
            "/api/v1/research",
            json={
                "query": f"Test query for {depth} depth research",
                "depth": depth
            }
        )
...
def test_get_existing_task_status(client: TestClient, sample_task_id: str):
    ...
    assert data["status"] in ["queued", "processing", "completed", "failed", "pending_review", "approved"]
...
def test_edit_review(client: TestClient, pending_review_task_id: str):
    ...
    edited_report = "This is an edited version of the research report with updated findings."
