...
class TestArxivIngestion:
    ...
    def test_full_ingestion_pipeline(self):
        ...
        ingestion = ArxivIngestion()
        summary = ingestion.run_pipeline(
            categories=['cs.AI'],
            max_results=5
        )
...
class TestPDFProcessing:
    ...
    def test_pdf_extraction(self):
        ...
        text = "\n\n".join([page['text'] for page in text_result['pages'] if page['text']])
        
        assert len(text) > 0, "Should extract text"
        assert len(text) > 100, "Should extract substantial text"
        
        print(f"   ✅ Extracted {len(text)} characters from {text_result['total_pages']} pages")
    ...
    def test_text_chunking(self):
        ...
        assert all(len(chunk.split()) <= 100 for chunk in chunks[:-1]), "Chunks should respect size limit"
    ...
    def test_complete_pdf_processing(self):
        ...
        result = processor.process_pdf(local_path, extract_tables=False, extract_layout=False)
        
        assert result['success'], "Processing should succeed"
        assert result['chunks']['num_chunks'] > 0, "Should create chunks"
        assert len(result['chunks']['chunks']) == result['chunks']['num_chunks'], "Chunk count should match"
        
        print(f"   ✅ Processed PDF:")
        print(f"      PDF name: {result['pdf_name']}")
        print(f"      Pages: {result['text_extraction']['total_pages']}")
        print(f"      Text length: {result['text_extraction']['full_text_length']:,} chars")
