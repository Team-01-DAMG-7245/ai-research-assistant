"""Test script to run HITL review agent"""
import json
import sys
...
def main():
    ...
    state: ResearchState = {
        'task_id': f'hitl_test_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
    ...
    print("\n" + "=" * 70)
    print("STEP 4: Running HITL Review")
    print("=" * 70)
    
    # Run HITL review
    state = hitl_review_node(state)
    ...
    output_file = Path(__file__).parent / "hitl_review_results.json"
